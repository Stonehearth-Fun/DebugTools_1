...
local SessionLogger = require 'debugtools_1_0.lib.session_logger'
...
function Commands:interrupt_ai(session, response, entity)
   ...
   radiant.events.listen(entity, 'debugtools_1_0:ai_interrupted', function()
               ...
               if timesNine == true then
                  for x=-1,1 do
                     for z=-1,1 do
                        ...
                        if x~=0 or z~=0 then
                           _radiant.call('debugtools_1_0:create_entity', uri, iconic, location + Point3(x,0,z), rotation)
                        end
                     end
                  end
               end

               _radiant.call('debugtools_1_0:create_entity', uri, iconic, location, rotation)
                  :done(function()
...
function Commands:ai_reconsider_entity_command(session, response, entity)
   stonehearth.ai:reconsider_entity(entity, 'debugtools_1_0 reconsider')
   SessionLogger.write('AI_RECONSIDER', 'reconsider entity requested', {
      player_id = session and session.player_id,
      entity = entity,
   })
...
function Commands:set_ai_log_override_command(session, response, level, entity)
   ...
   SessionLogger.write('AI_LOG_OVERRIDE', 'ai log override updated', {
      player_id = session and session.player_id,
      level = level,
      entity = entity,
   })
...
function Commands:print_ai_stack_command(session, response, entity)
   ...
   if not trace_back then
      SessionLogger.warn('AI_STACK', 'traceback missing', {
         player_id = session and session.player_id,
         entity = entity,
      })
   ...
   SessionLogger.write('AI_STACK', 'ai stack reported', {
      player_id = session and session.player_id,
      entity = entity,
      traceback = string.gsub(trace_back, '%s+', ' '),
   })
...
function Commands:exec_script(session, response, script, entity)
   ...
   if error then
      SessionLogger.warn('SCRIPT_EXEC', 'script compilation failed', {
         player_id = session and session.player_id,
         entity = entity,
         error = error,
         script = script,
   ...
   if success then
      ...
      SessionLogger.write('SCRIPT_EXEC', 'script executed', {
         player_id = session and session.player_id,
         entity = entity,
         script = script,
         output = output,
      ...
      SessionLogger.error('SCRIPT_EXEC', 'script runtime error', {
         player_id = session and session.player_id,
         entity = entity,
         script = script,
         error = result,
